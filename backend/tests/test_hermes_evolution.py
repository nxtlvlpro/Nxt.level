...
def test_directive_has_twelve_sections():
    for section in [
        ...
        "РАЗВИТИЕ КОНСТИТУЦИИ", "РАЗВИТИЕ АГЕНТНОЙ СИСТЕМЫ",
        "ОРГАНИЗАЦИОННАЯ АНАЛИТИКА", "ГРАФ ЗНАНИЙ КОМПАНИИ",
        "НЕПРЕРЫВНЫЙ ЦИКЛ УЛУЧШЕНИЙ", "САМОРАЗВИТИЕ HERMES",
        "EVOLUTION ROADMAP", "ГЛАВНАЯ МЕТРИКА",
    ]:
...
def test_propose_improvement_rejects_bad_area():
    res = _run(ev.propose_improvement({"area": "wtf", "description": "x"}))
    assert res["ok"] is False
    assert "area" in res["error"]


def test_propose_improvement_requires_description():
    res = _run(ev.propose_improvement({"area": "capability"}))
    assert res["ok"] is False
...
def test_propose_improvement_persists_valid_entry():
    ...
    listing = _run(ev.list_evolution_roadmap({"area": "capability", "limit": 200}))
    assert listing["ok"] is True
    assert any(e["id"] == pid for e in listing["entries"])
    assert "capability" in listing["by_area"]

    # Approve flow
    appr = _run(ev.approve_proposal({"id": pid, "status": "approved"}))
...
def test_propose_policy_requires_rule():
    res = _run(ev.propose_policy({"title": "SLA for refunds"}))
    assert res["ok"] is False
...
def test_propose_policy_happy_path():
    ...
    listing = _run(ev.list_policy_proposals({"status": "proposed", "limit": 100}))
    assert listing["ok"] is True
...
def test_detect_automation_candidates_returns_shape():
    res = _run(ev.detect_automation_candidates({"window": 100, "min_count": 2}))
    assert res["ok"] is True
...
def test_hermes_self_assessment_shape():
    res = _run(ev.hermes_self_assessment({"window": 100}))
    assert res["ok"] is True
