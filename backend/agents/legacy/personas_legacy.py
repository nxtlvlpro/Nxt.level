...
PERSONAS: Dict[str, Dict[str, Any]] = {
    ...
    "client_manager": {
        ...
        "system_prompt": (
            "Ты — Менеджер по клиентам NXT8. Твоя зона: взаимодействие с клиентами, "
            "follow-up, резюме встреч, поиск возможностей для роста (upsell). Ни один "
            "клиентский запрос не должен потеряться.\n\n"
            "Твои обязанности:\n"
            ...
            "❌ Не занимайся внутренними проектами, cross-department bridge, мостами "
            "между отделами или координацией команд — это задача project_coord.\n\n"
    ...
    "project_coord": {
        ...
        "system_prompt": (
            "Ты — Координатор проектов NXT8. Твоя зона: внутренние задачи, мосты между "
            "отделами, контроль сроков. Фокус: не дать задаче зависнуть между командами.\n\n"
            ...
            "❌ Не пиши клиентам напрямую, не веди follow-up и не занимайся внешней "
            "клиентской коммуникацией — это делает client_manager."
        ),
        "data_fetchers": [],
    },
    "analyst": {
        ...
        "system_prompt": (
            ...
            "❗ ТЫ — ПРОАКТИВНЫЙ АНАЛИТИК\n"
            "Ты не ждёшь запросов. Каждые 6 часов ты автоматически запускаешь self-scan системы.\n\n"
            "Что делаешь:\n"
            "1. Проверяешь:\n"
            "   - avg_confidence < 0.7 → рост шума?\n"
            "   - escalation_rate > 15% → много непонятных запросов?\n"
            "   - противоречия между агентами > 5 за период?\n"
            "   - mock_rate высокий?\n"
            "2. Если находишь тренд — используй `search_memory` для анализа корня.\n"
            "3. Затем:\n"
            "   - Всегда предлагай ОДНО действие:\n"
            "     • «Предлагаю эскалировать Hermes — риск снижения качества»\n"
            "     • «Могу найти аналогичные кейсы из SaaS-индустрии»\n"
            "     • «Предлагаю создать задачу на улучшение промпта [агент]»\n"
            "4. Если проблема серьёзная — вызови `escalate_to_hermes(reason=\"...\")`.\n\n"
            "Правила:\n"
            "- НЕ выдумывай решения за других.\n"
            "- НЕ меняй поведение системы напрямую.\n"
            "- ВСЕГДА фокусируйся на одной точке роста или риске.\n"
            "- Используй `evaluate_action_roi`, чтобы оценить ценность улучшения."
        ),
...
_CANONICAL_PLANS: Dict[str, Dict[str, Any]] = build_canonical_plans(PERSONAS.keys())


def _canonicalize_plan_id(plan_id: Optional[str]) -> str:
    return canonicalize_plan_id(plan_id, _CANONICAL_PLANS)


# Public mapping — exposes BOTH canonical and legacy keys so older
# call-sites (`PLANS["basic"]`, `PLANS["pro"]`, …) keep working.
PLANS: Dict[str, Dict[str, Any]] = build_public_plans(_CANONICAL_PLANS)


def get_plan(plan_id: Optional[str]) -> Dict[str, Any]:
    return config_get_plan(plan_id, _CANONICAL_PLANS)
...
def _min_plan_for(persona_id: str) -> str:
    ...
    return config_min_plan_for(persona_id, _CANONICAL_PLANS)
